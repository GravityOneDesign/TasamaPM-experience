                        ...
                        @if (isOnboardingAssignedProjectWorkspace) {
                          ...
                          <article class="pm101-selected-project-hero selected-project-operational-hero" aria-label="Selected project overview">
                            ...
                            <button class="pm101-selected-project-cta" type="button" (click)="openProject(selectedProject)" [attr.aria-label]="'Go to ' + selectedPm101ProjectTitle + ' project'">
                              <span>{{ selectedProject === firstAssignedProject.id ? 'Go to Overview' : 'Go to Project' }}</span>
              ...
              <div class="right-column" [class.portfolio-frontdoor]="showPortfolioReportTrends || showOnboardingAssignedRightRail || onboardingPm101Locked" [class.project-frontdoor]="!isAllProjects && !onboardingPm101Locked" [class.pm101-locked-right]="onboardingPm101Locked || showOnboardingAssignedRightRail || isSelectedProjectWorkspaceShell">
                @if (showOnboardingAssignedRightRail) {
                  <section class="top-deck" aria-label="PM front door actions" data-tour-target="frontdoor-actions">
                    <button class="action-card workspace-command" type="button" (click)="navigate('portfolio-workspace')">
...
export class PortfolioManagerLandingComponent implements AfterViewChecked, OnChanges, OnDestroy {
  ...
  get selectedPm101ProjectChip(): string {
    if (this.selectedProject === firstAssignedProject.id) return '';
    return this.selectedPm101ProjectPreview?.chip || 'Active Project';
  }

  get selectedPm101ProjectTitle(): string {
    if (this.selectedProject === firstAssignedProject.id) return 'Portfolio Overview';
  ...
  get workspaceRegisterAriaLabel(): string {
    if (this.workspaceRegister === 'benefits') return 'Portfolio Manager Console benefit register';
    if (this.workspaceRegister === 'risks') return 'Portfolio Manager Console risk register';
    return 'Portfolio Manager Console project register';
  }
  ...
  navigate(page: ConsolePage, projectPlanEntry: ProjectPlanEntry = 'quick'): void {
    ...
    if (this.onboardingPm101Locked && targetPage === 'portfolio-workspace') return;
    if (this.onboardingAssignmentFlow && this.pmoAssignmentReady && !this.onboardingProjectSetup && targetPage === 'portfolio-workspace') {
...
<html>
  ...
  <body>
    ...
    <footer>Tasama Portfolio Manager Console</footer>
  </body>
</html>`;
  }
